...
function initApp() {
  ...
  function parseResume(text) {
    ...
    // Name: First line with 2-4 words that are all letters, excluding known section headers
    const headers = ['education', 'profile', 'skills', 'experience', 'work', 'summary', 'objective', 'contact', 'phone', 'email', 'address', 'college', 'school', 'university', 'institute', 'academy', 'global', 'trinity', 'islington', 'galaxy', 'diamond', 'global academy', 'technical', 'soft', 'intern', 'freelancing', 'motion', 'graphic', 'designer', 'internship', 'marketing', 'multimedia', 'secondary', 'bachelors'];
    for (const line of lines) {
      const lower = line.toLowerCase();
      const words = line.split(/\s+/);
      // Check: 2-4 words, all alphabetic, each word 2+ chars
      if (words.length >= 2 && words.length <= 4 && words.every(w => /^[A-Za-z]+$/.test(w) && w.length >= 2)) {
        // Check it's not a section header or institution name
        let isHeader = false;
        for (const h of headers) {
          if (lower.includes(h)) { isHeader = true; break; }
        }
        if (!isHeader) {
          data.name = line;
          break;
    ...
    // Summary: text after Profile/Summary label
    ...
    // Skills: collect all lines after Skills/Technical Skills/Soft Skills until next section
    ...
    // Experience: match specific job titles/roles
    const expKeywords = ['internship', 'freelancing', 'intern', 'marketing', 'designer', 'editor', 'developer'];
    for (const line of lines) {
      const lower = line.toLowerCase();
      let matched = false;
      for (const kw of expKeywords) {
        if (lower.includes(kw) && !lower.includes('education') && !lower.includes('college') && !lower.includes('school')) {
          data.experience.push(line);
          const idx = lines.indexOf(line);
          if (idx + 1 < lines.length) data.experience.push(lines[idx + 1]);
          if (idx + 2 < lines.length && lines[idx + 2].length > 20) data.experience.push(lines[idx + 2]);
          matched = true;
          break;
        }
      }
      if (matched && lower.includes('freelancing')) {
        // Add following lines for freelancing
        const idx = lines.indexOf(line);
        if (idx + 1 < lines.length) data.experience.push(lines[idx + 1]);
    ...
    // Education: lines with education keywords (not in experience context)
    const eduKeywords = ['bachelors', 'secondary education', 'college', 'university', 'diploma', 'degree', 'school'];
    for (const line of lines) {
      const lower = line.toLowerCase();
      for (const kw of eduKeywords) {
        if (lower.includes(kw) && !data.education.includes(line)) {
          data.education.push(line);
          const idx = lines.indexOf(line);
          if (idx + 1 < lines.length && !lines[idx + 1].toLowerCase().match(/^(skills|experience|work|profile|summary)$/)) {
  ...
  function generatePortfolioHTML(data) {
    ...
    let skillsHTML = '';
    skills.forEach(skill => { skillsHTML += '<div class="skill-card"><h3>' + skill + '</h3><p>Professional skill</p></div>'; });
    let expHTML = '';
    if (data.experience.length > 0) { for (let i = 0; i < data.experience.length; i++) { expHTML += '<li>' + data.experience[i] + '</li>'; } }
    let eduHTML = '';
    if (data.education.length > 0) { for (let i = 0; i < data.education.length; i++) { eduHTML += '<li>' + data.education[i] + '</li>'; } }
  ...
  downloadBtn.onclick = () => { if (currentPortfolioHTML) downloadPortfolio(currentPortfolioHTML, currentName); };
  openBtn.onclick = () => { if (currentPortfolioHTML) openPortfolio(currentPortfolioHTML); };
  generateBtn.onclick = () => {
  ...
  uploadCard.addEventListener('drop', (e) => { e.preventDefault(); uploadCard.classList.remove('dragover'); const files = e.dataTransfer.files; if (files.length > 0) { resumeFile.files = files; generatePortfolio(); } });
  resumeFile.addEventListener('change', () => { if (resumeFile.files.length > 0) generatePortfolio(); });
  uploadCard.addEventListener('click', (e) => { if (e.target !== resumeFile && e.target !== pasteArea && e.target !== generateBtn) resumeFile.click(); });
}
