...
<body>
  ...
  function processText(text) {
    ...
    try {
      ...
      console.log('Portfolio generated for:', currentName);
  ...
  downloadBtn.onclick = () => {
    if (currentPortfolioHTML) {
      downloadPortfolio(currentPortfolioHTML, currentName);
    }
  };

  openBtn.onclick = () => {
    if (currentPortfolioHTML) {
      openPortfolio(currentPortfolioHTML);
    }
  };
  ...
  generateBtn.onclick = () => {
    ...
    if (text) {
      processText(text);
    } else if (resumeFile.files.length > 0) {
      generatePortfolio();
    } else {
      statusMessage.textContent = 'Please upload a file or paste your resume text.';
    }
  };
  ...
  uploadCard.addEventListener('click', (e) => {
    if (e.target !== resumeFile && e.target !== pasteArea && e.target !== generateBtn) {
      resumeFile.click();
    }
  });
}
